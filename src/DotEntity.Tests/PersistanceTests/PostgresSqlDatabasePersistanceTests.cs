...
{
    [TestFixture]
    public class PostgresSqlDatabasePersistanceTests : DotEntityTest
    {
        [OneTimeSetUp]
        public void Init()
        {
            DotEntityDb.Initialize(PostgreSqlConnectionString, new PostgreSqlDatabaseProvider());
        ...
        [Test]
        public void PostgreSql_Direct_Count_Succeeds()
        {
        ...
        [Test]
        public void PostgreSqlInsert_Succeeds()
        {
            var product = new Product()
            {
                ProductName = "PostgreSqlInsert_Succeeds",
            ...
            var savedProduct = EntitySet<Product>.Where(x => x.ProductName == "PostgreSqlInsert_Succeeds").SelectSingle();
            Assert.AreEqual(product.Id, savedProduct.Id);
        }

        [Test]
        public void PostgreSql_Select_Simple_Succeeds()
        {
            var productCount = 5;
            ...
            for (var i = 0; i < productCount; i++)
            {
                ...
                {
                    ProductName = $"PostgreSql_Select_Simple_Succeeds {i + 1}",
            ...
            var savedProducts = EntitySet<Product>.Where(x => x.ProductName.Contains("PostgreSql_Select_Simple_Succeeds"))
                .Select();
        ...
        [Test]
        public void PostgreSql_Select_Count_Succeeds()
        {
            var product1 = new Product()
            {
                ProductName = "PostgreSql_Select_Count_Succeeds One",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 10
            };
            ...
            {
                ProductName = "PostgreSql_Select_Count_Succeeds Two",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 20
            };
            ...
            var cnt = EntitySet<Product>.Where(x => x.ProductName.Contains("PostgreSql_Select_Count_Succeeds")).Count();

            Assert.AreEqual(2, cnt);
        }

        [Test]
        public void PostgreSql_Select_With_Matching_Count_Succeeds()
        {
            var product1 = new Product()
            {
                ProductName = "PostgreSql_Select_With_Matching_Count_Succeeds One",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 10
            };
            ...
            var product2 = new Product()
            {
                ProductName = "PostgreSql_Select_With_Matching_Count_Succeeds Two",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 20
            };
            ...
            var products = EntitySet<Product>
                .Where(x => x.ProductName.Contains("PostgreSql_Select_With_Matching_Count_Succeeds"))
                .OrderBy(x => x.Id)
        ...
        [Test]
        public void PostgreSql_SingleQuery_Succeeds()
        {
            var productCount = 5;
            ...
            for (var i = 0; i < productCount; i++)
            {
                ps.Add(new Product()
                {
                    ProductName = $"PostgreSql_SingleQuery_Succeeds {i + 1}",
                    ProductDescription = $"Some descriptoin won't hurt {i + 1}",
                    DateCreated = DateTime.Now,
                    Price = 10
                });
            ...
            var products = EntitySet<Product>.Query(@"SELECT * FROM ""Product"" WHERE ""ProductName"" LIKE @ProductName;",
                new {ProductName = "%PostgreSql_SingleQuery_Succeeds%"}).ToList();
        ...
        [Test]
        public void PostgreSql_MultiQuery_Succeeds()
        {
            var product1 = new Product()
            {
                ProductName = "PostgreSql_MultiQuery_Succeeds One",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 10
            };
            ...
            var product2 = new Product()
            {
                ProductName = "PostgreSql_MultiQuery_Succeeds Two",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 20
            };
            ...
            using (var result = EntitySet.Query(@"SELECT * FROM ""Product"" WHERE ""ProductName"" LIKE @ProductName;
                        SELECT COUNT(*) FROM ""Product"" WHERE ""ProductName"" LIKE @ProductName;", new { ProductName = "%PostgreSql_MultiQuery_Succeeds%" }))
            {
        ...
        [Test]
        public void PostgreSql_Select_Join_Succeeds()
        {
            var product1 = new Product()
            {
                ProductName = "PostgreSql_Select_Join_Succeeds One",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 10
            };
            ...
            var product2 = new Product()
            {
                ProductName = "PostgreSql_Select_Join_Succeeds Two",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 20
            };
            ...
            {
                ProductName = "PostgreSql_Select_Join_Succeeds Three",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 20
            };
            ...
            var products = EntitySet<Product>
                .Where(x => x.ProductName.Contains("PostgreSql_Select_Join_Succeeds"))
        ...
        [Test]
        public void PostgreSql_Update_Succeeds()
        {
            var product = new Product()
            {
                ProductName = "PostgreSqlUpdate_Succeeds before",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 25
            };
            ...
            product.ProductName = "PostgreSqlUpdate_Succeeds after";
            product.Price = 50;
            EntitySet<Product>.Update(product);

            var savedProduct = EntitySet<Product>.Where(x => x.Id == id).SelectSingle();
            Assert.AreEqual("PostgreSqlUpdate_Succeeds after", savedProduct.ProductName);
            Assert.AreEqual(50, savedProduct.Price);
        }

        [Test]
        public void PostgreSql_Update_Dynamic_Fields_Succeeds()
        {
            var product1 = new Product()
            {
                ProductName = "PostgreSql_Select_Simple_Succeeds One",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 10
            };
            ...
            var product2 = new Product()
            {
                ProductName = "PostgreSql_Select_Simple_Succeeds Two",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 20
            };
            ...
            EntitySet<Product>.Insert(new[] { product1, product2 });
            var id = product2.Id;
            EntitySet<Product>.Update(new {ProductName = "PostgreSql_Select_Simple_Succeeds Two Modified", Price = 50}, x => x.Id == id);

            var savedObject = EntitySet<Product>.Where(x => x.Id == id).SelectSingle();

            Assert.AreEqual("PostgreSql_Select_Simple_Succeeds Two Modified", savedObject.ProductName);
            Assert.AreEqual(50, savedObject.Price);
        }

        [Test]
        public void PostgreSql_Delete_Succeeds()
        {
            var product = new Product()
            {
                ProductName = "PostgreSql_Delete_Succeeds",
                ProductDescription = "Some descriptoin won't hurt",
                DateCreated = DateTime.Now,
                Price = 25
            };
        ...
        public void Index_Tests_Succeeds()
        {
            ...
            {
                DotEntity.Database.CreateIndex<Customer>(new string[] { nameof(Customer.Name) }, null, transaction);
                transaction.Commit();
            }

            using (var transaction = new DotEntityTransaction())
            {
                DotEntity.Database.DropIndex<Customer>(new string[] { nameof(Customer.Name) }, transaction);
                transaction.Commit();
            }
