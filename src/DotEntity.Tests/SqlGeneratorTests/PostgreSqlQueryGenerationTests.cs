...
{
    [TestFixture]
    public class PostgreSqlQueryGenerationTests : DotEntityTest
    {
        private readonly IQueryGenerator generator;

        public PostgreSqlQueryGenerationTests()
        {
            DotEntityDb.Initialize(PostgreSqlConnectionString, new PostgreSqlDatabaseProvider("testdb"), SelectQueryMode.Wildcard);
        ...
        public void SelectGeneration_WithoutAnything_Paginated_Valid()
        {
            ...
            var expected = "SELECT * FROM \"Product\" ORDER BY \"Id\" LIMIT 30 OFFSET 0;";
            Assert.AreEqual(expected, sql);
            Assert.AreEqual(0, queryParameters.Count);
        }
        ...
        public void SelectGeneration_With_Where_Paginated_Valid()
        {
            ...
            var expected = "SELECT * FROM \"Product\" WHERE (\"ProductName\" = @ProductName) ORDER BY \"Id\" LIMIT 30 OFFSET 0;";
            Assert.AreEqual(expected, sql);
        ...
        public void InsertGenerator_EntityType_Valid()
        {
            ...
            var expected = "INSERT INTO \"Product\" (\"ProductName\",\"ProductDescription\",\"DateCreated\",\"Price\",\"IsActive\") VALUES (@ProductName,@ProductDescription,@DateCreated,@Price,@IsActive) RETURNING \"Id\";";
        ...
        public void InsertGenerator_EntityType_WithSchema_Valid()
        {
            var p = new Customer();
            var sql = generator.GenerateInsert(p, out IList<QueryInfo> queryParameters);
            var expected = "INSERT INTO \"xyz\".\"Customer\" (\"Name\",\"Uid\") VALUES (@Name,@Uid) RETURNING \"CustomerId\";";
