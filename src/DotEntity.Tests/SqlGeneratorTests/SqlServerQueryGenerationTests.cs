...
{
    [TestFixture]
    public class SqlServerQueryGenerationTests : DotEntityTest
    {
        ...
        public void JoinGenerator_Simple_Valid()
        {
            ...
            var expected = "SELECT * FROM (SELECT * FROM [Product] t1) AS t1 INNER JOIN [ProductCategory] t2 ON t1.[Id] = t2.[ProductId] INNER JOIN [Category] t3 ON t2.[CategoryId] = t3.[Id];";
            Assert.AreEqual(expected, sql);
            Assert.AreEqual(0, queryParameters.Count);
        }
        ...
        public void JoinGenerator_Simple_With_Additional_Join_Expression_Valid()
        {
            ...
            var expected = "SELECT * FROM (SELECT * FROM [Product] t1) AS t1 INNER JOIN [ProductCategory] t2 ON t1.[Id] = t2.[ProductId] INNER JOIN [Category] t3 ON t2.[CategoryId] = t3.[Id] AND t3.[CategoryName] = @t3_CategoryName;";
            Assert.AreEqual(expected, sql);
            Assert.AreEqual(1, queryParameters.Count);
        }
        ...
        public void JoinGenerator_Simple_WithParentSource_Valid()
        {
            ...
            var expected = "SELECT * FROM (SELECT * FROM [Product] t1) AS t1 INNER JOIN [ProductCategory] t2 ON t1.[Id] = t2.[ProductId] INNER JOIN [Category] t3 ON t1.[CategoryId] = t3.[Id];";
            Assert.AreEqual(expected, sql);
            Assert.AreEqual(0, queryParameters.Count);
        }
        ...
        public void JoinGenerator_Simple_Where_Valid()
        {
            ...
            var expected = "SELECT * FROM (SELECT * FROM [Product] t1) AS t1 INNER JOIN [ProductCategory] t2 ON t1.[Id] = t2.[ProductId] INNER JOIN [Category] t3 ON t2.[CategoryId] = t3.[Id]  WHERE (t1.[Id] = t3.[Id]);";
            Assert.AreEqual(expected, sql);
            Assert.AreEqual(1, queryParameters.Count);
        }
        ...
        public void JoinGenerator_Multiple_Where_Valid()
        {
            ...
            var expected = "SELECT * FROM (SELECT * FROM [Product] t1) AS t1 LEFT OUTER JOIN [ProductCategory] t2 ON t1.[Id] = t2.[ProductId] INNER JOIN [Category] t3 ON t2.[CategoryId] = t3.[Id]  WHERE (t1.[Id] = t3.[Id]) AND (t2.[CategoryId] > t3.[Id]);";
            Assert.AreEqual(expected, sql);
            Assert.AreEqual(2, queryParameters.Count);
        }
    }
